...
class Exercise5 < Test::Unit::TestCase
  def test_register_saves
    ...
    float_assert("3.0", "{R5=1.5;R1*2;}", "Simple float register save")
  end

  def test_correct_register_usage
    (0..4).each do |i|
      float_not_assert("6", "{R#{i}=2;R#{i}*3;}", "Simple improper integer register#{i} save")
    end
    (5..9).each do |i|
      float_not_assert("3.0", "{R#{i}=1.5;R#{i}*2;}", "Simple improper float register#{i} save")
    end
  end
